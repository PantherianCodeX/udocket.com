...
def test_docs_lint_script_runs_on_repo() -> None:
    cmd = ["python", str(REPO_ROOT / "scripts" / "docs" / "lint_docs.py")]
...
def test_mkdocs_build_produces_expected_pages(built_site: Path) -> None:
    expected_paths = [
        ...
        built_site / "services" / "lp-engine.html",
        built_site / "services" / "settings.html",
    ]
...
def test_preamble_divider_convention_preserved() -> None:
    ...
    template = DOCS_SRC / "services" / "_template.md"
