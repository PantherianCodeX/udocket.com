...
To share wheel layers between devcontainer and host builds, switch to a
container-based Buildx builder **before** running `docker compose build`:

```bash
docker buildx create --name udocket --driver docker-container --bootstrap --use
```

The compose files export cache layers under `.docker/buildx-cache/` using
per-service directories to avoid write races:

- Devcontainer: `.docker/buildx-cache/dev`
- Platform (web): `.docker/buildx-cache/platform`
- Worker: `.docker/buildx-cache/platform_worker`
- Beat: `.docker/buildx-cache/platform_beat`

Tip: create them once to avoid first-run warnings:

```bash
mkdir -p .docker/buildx-cache/{dev,platform,platform_worker,platform_beat}
```

With the `udocket` builder active, both devcontainer and host builds reuse
these caches automatically. To export (write) caches, use the override file:

```bash
docker compose -f docker-compose.yml -f docker-compose.cache.yml build
```

Tip: on day-to-day builds, skip the override to avoid large cache write I/O;
the build will still read from existing caches.
