...
docs/                              ← single doc root
  ...
  site/
  config/
    .markdownlint.json (extends packages/docs_tooling/config/.markdownlint.json)
    mermaid.config.json
    mermaidrc.json
    mkdocs.yml
...
Path rule: if a source lives at `docs/<REL>/diagrams/<NAME>.mmd`, the built SVG is at `packages/docs_tooling/build/diagrams/<REL>/<NAME>.svg`. Embed using `/build/diagrams/<REL>/<NAME>.svg` so links stay valid from any document depth.
...
* **MkDocs/Nav:** Update `packages/docs_tooling/mkdocs.yml` navigation to point to `overview/tdd.md`, `overview/tdd/*`, and `overview/prd/*` alongside `services/` and `apps/`.
...
`packages/docs_tooling/mkdocs.yml` (essentials you’ll likely want):
...
**packages/docs_tooling/config/vale.ini:**
...
jobs:
  lint:
    ...
    steps:
      ...
      - name: Markdownlint
        run: npx markdownlint --config packages/docs_tooling/config/.markdownlint.json '**/*.md' '#node_modules'
      - name: Vale
        run: vale --config packages/docs_tooling/config/vale.ini docs/
      - name: Mermaid pre-render (validate diagrams)
        run: uv run --project packages/docs_tooling python -m doc_tools.render_mermaid
  ...
  build-and-release:
    ...
    steps:
      ...
      - name: Upload PDFs
        ...
        with:
          files: |
            out/doc-builds/pdf/dev/prd.pdf
            out/doc-builds/pdf/dev/tdd.pdf
            out/doc-builds/pdf/dev/manifest.json
```
...
1. **Create structure & config**
    ...
    * Ensure `.markdownlint.json` (extending `packages/docs_tooling/config/.markdownlint.json`), `packages/docs_tooling/mkdocs.yml`, `packages/docs_tooling/config/.markdownlint.json`, optional `packages/docs_tooling/config/vale.ini` exist.
...
3. **Cross-link cleanup**
    ...
    * Ensure link paths match `packages/docs_tooling/mkdocs.yml` nav.
