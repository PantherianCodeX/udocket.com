...
def parse_svg(path: Path) -> SvgTree | None:
    if not path.exists():
        return None
...
def center_edge_labels(root: SvgElement) -> bool:
    line_height_em = 1.1
    pad_x = 8.0
    pad_y = 4.0
    changed = False
...
def wrap_sequence_actor_labels(root: SvgElement) -> bool:
    changed = False
    line_height_em = 1.1
    avg_char_px = 8.0
    padding_px = 16.0
...
def ensure_rect_radius(root: SvgElement, radius: float = 12.0) -> bool:
    changed = False
...
def make_rounded_backplates(root: SvgElement, radius: float = 12.0) -> bool:
    changed = False

    def eligible_container(path: SvgElement) -> SvgElement | None:
        for ancestor in path.iterancestors(f"{SVG}g"):
            ...
            if any(
                ...
                return ancestor
        return None
    def process_path(path: SvgElement, container: SvgElement, radius_multiplier: float = 1.0) -> None:
        nonlocal changed
        try:
