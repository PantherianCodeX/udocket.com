...
from .base import SchemeBundle, IdentifierScheme
...
def schemes_by_court(root: Optional[str] = None) -> Dict[str, List[IdentifierScheme]]:
    idx: Dict[str, List[IdentifierScheme]] = {}
    for b in discover_scheme_bundles(root):
        for s in b.data:
            idx.setdefault(s.court_key, []).append(s)
    return idx

def all_schemes(root: Optional[str] = None) -> List[IdentifierScheme]:
    out: List[IdentifierScheme] = []
    for b in discover_scheme_bundles(root):
        out.extend(b.data)
    return out
