...
- **Behaviour**:
  - Seeds the run with curated distributions (Django, DRF, redis, requests, etc.) and any extra `--dist` arguments.
  ...
  - Skips work entirely when `typings/vendor/` already contains files unless `--force` is supplied, allowing compose/dev environments to auto-populate the directory on first run without polluting git history.
  - When files are generated, records results in `typings/vendor/VENDORED_STUBS.json` and updates the automation manifest helper record for traceability.
- **Idempotency**: Each run replaces the vendored directories atomically (and the directory is ignored by git), so repeated executions with unchanged inputs produce identical trees. Use `--force` when a regeneration is required.
