...
- Use the unified doc manager (`uv run python -m docs.tools.manage_docs`) or the Make targets (`make lint-docs`, `make build-docs`) to run common workflows:
  - `uv run python -m docs.tools.manage_docs --lint` — read-only checks
  - `uv run python -m docs.tools.manage_docs --sync` — regenerate runbook/diagram/SLO/API appendices and sync document controls/assets
  - `uv run python -m docs.tools.manage_docs --build` — strict MkDocs build (temporary site dir on `--dry-run`)
  - `uv run python -m docs.tools.manage_docs --pdf` — produce TDD/PRD PDFs
  - Combine flags for bespoke flows, e.g. `uv run python -m docs.tools.manage_docs --lint --sync --dry-run` or `uv run python -m docs.tools.manage_docs --all`
...
- Validate service specs against the template: `uv run python -m docs.tools.check_structure docs/src/platform docs/src/automation docs/src/data docs/src/customer`
...
- Build site: `uv run mkdocs build --config-file mkdocs.yml --site-dir site --clean`.
- Preview docs locally (default dev server port 8010):
  - `uv run mkdocs serve --config-file mkdocs.yml --dev-addr 0.0.0.0:8010`
- Build TDD/PRD PDFs:
  - `bash tools/render_mermaid.sh --all`
