...
- **Maintenance:** Run `python -m docs.tools.manage_docs --lint docs/src/platform/runtime.md docs/src/overview/tdd.md docs/tdd_modularization.md` plus `python -m docs.tools.build.runbook_catalog --check` before landing infra changes. Update diagrams via `docs/tools/render_mermaid.sh` whenever topology shifts.
