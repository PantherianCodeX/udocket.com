...
- **Maintenance:** Run `python -m docs.tools.manage_docs --lint docs/src/customer/communications.md docs/src/overview/tdd.md docs/tdd_modularization.md` before submitting changes. Updates that alter schema, queue semantics, or provider adapters also require `python -m docs.tools.build.runbook_catalog --check` to pass. Notify Platform + Ops architecture lists on PRs.
...
**Observability:** Docs lint (`python -m docs.tools.build.runbook_catalog --check`), dashboards “Notifications Delivery” / “In-App Notifications”, alert `alert_notifications_delivery_health`. **|**
