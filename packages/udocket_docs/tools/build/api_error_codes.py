...
def _load_entries(yaml_path: Path) -> list[ErrorCodeEntry]:
    ...
    if not isinstance(codes_obj, list) or not codes_obj:
...
def build_content(*, check: bool) -> bool:
    ...
    if not write_or_check(
        ...
        new_appendix,
        check=check,
        stale_message="API error codes tables are stale; run `python -m docs.tools.build.api_error_codes`.",
    ):
        stale = True
    return not stale
...
def _run(argv: Sequence[str] | None = None) -> int:
    ...
    if args.check and not ok:
        print("API error codes tables are stale; run `python -m docs.tools.build.api_error_codes`.", file=sys.stderr)
        return 1
    return 0
