...
VALE_CI_CONFIG = str((ROOT / "docs" / "config" / "vale-ci.ini").relative_to(ROOT))
VALE_TARGETS = [
    str((ROOT / "docs" / "src" / "_template.md").relative_to(ROOT)),
    str((ROOT / "docs" / "src" / "experience" / "_template.md").relative_to(ROOT)),
    str((ROOT / "docs" / "tests" / "vale_fixtures" / "service_valid.md").relative_to(ROOT)),
]
...
def builder_runbook_check(_: RunContext) -> list[str]:
    return python_task("docs.tools.build.runbook_catalog", "--check")


def builder_diagram_check(_: RunContext) -> list[str]:
    return python_task("docs.tools.build.diagram_index", "--check")


def builder_api_error_codes_check(_: RunContext) -> list[str]:
    return python_task("docs.tools.build.api_error_codes", "--check")


def builder_slo_check(_: RunContext) -> list[str]:
    return python_task("docs.tools.build.slo_index", "--check")
...
def builder_check_appendices(_: RunContext) -> list[str]:
    return python_task("docs.tools.check_appendices")


def builder_markdownlint(_: RunContext) -> list[str]:
    config = str((ROOT / "docs" / "config" / ".markdownlint.json").relative_to(ROOT))
    return [
        "npx",
        "markdownlint-cli",
        "--config",
...
def builder_check_settings(_: RunContext) -> list[str]:
    return python_task("docs.tools.check_settings_keys")


def builder_check_links(_: RunContext) -> list[str]:
    return python_task("docs.tools.check_links")


def builder_sync_doc_controls(ctx: RunContext) -> list[str]:
    cmd = python_task("docs.tools.sync.document_controls")
    if ctx.targets:
...
def builder_sync_doc_assets(ctx: RunContext) -> list[str]:
    cmd = python_task("docs.tools.sync.doc_assets")
    if ctx.dry_run:
        cmd.append("--dry-run")
    return cmd
...
def builder_runbook_update(ctx: RunContext) -> list[str]:
    if ctx.dry_run:
        return python_task("docs.tools.build.runbook_catalog", "--check")
    return python_task("docs.tools.build.runbook_catalog")


def builder_diagram_update(ctx: RunContext) -> list[str]:
    if ctx.dry_run:
        return python_task("docs.tools.build.diagram_index", "--check")
    return python_task("docs.tools.build.diagram_index")


def builder_slo_update(ctx: RunContext) -> list[str]:
    cmd = ["docs.tools.build.slo_index"]
    if ctx.dry_run:
...
def builder_api_error_update(ctx: RunContext) -> list[str]:
    if ctx.dry_run:
        return python_task("docs.tools.build.api_error_codes", "--check")
    return python_task("docs.tools.build.api_error_codes")


def builder_mkdocs(ctx: RunContext) -> list[str]:
    cmd = python_task("docs.tools.build.mkdocs")
    if ctx.dry_run:
        cmd.append("--dry-run")
    return cmd
...
def builder_pdf_tdd(ctx: RunContext) -> list[str] | None:
    if ctx.dry_run:
        ...
        return None
    return ["bash", "-lc", "docs/tools/build/pdf_tdd.sh"]
...
def builder_pdf_prd(ctx: RunContext) -> list[str] | None:
    if ctx.dry_run:
        ...
        return None
    return ["bash", "-lc", "docs/tools/build/pdf_prd.sh"]
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        "targets",
        nargs="*",
        help="Optional paths to scope lint/sync commands (defaults to docs/src).",
    )
