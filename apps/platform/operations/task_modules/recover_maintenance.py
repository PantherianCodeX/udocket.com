...
_ACTIVE_META_STATUSES = {"running", "started", "processing", "active"}
...
def recover_stale_jobs(self: TaskProtocol) -> dict[str, object]:
    ...
    if not candidates:
        return {"status": "ok", "checked": 0, "resumed": 0, "finalized": 0, "skipped": 0}
    ...
    finalized = 0
    skipped = 0
    inspection_available = inspect_obj is not None
    ...
    for job in candidates:
        ...
        if (
            not inspection_available
            and task_ids
            and (job.status in {Job.Status.RUNNING, Job.Status.UPLOADING, Job.Status.CONVERTING})
            and task_status in _ACTIVE_META_STATUSES
        ):
            log.debug(
                "skip job recovery: worker state unknown",
                extra={
                    "job_id": job_id,
                    "case_id": case_id,
            ...
            safe_job_meta(
                ...
                job_id,
                {
                    ...
                    "recovery_error": "worker_state_unknown",
