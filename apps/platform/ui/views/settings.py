...
def _infer_provider_key(raw_key: str, endpoint: str) -> str:
    ...
    if key in load_provider_catalog().keys() or key in {
        "azure",
        "openai",
        "anthropic",
        "google",
...
def organization_settings(request: HttpRequest, section: str | None = None) -> HttpResponse:
    ...
    if request.method == "POST":
        post_section = request.POST.get("section") or request.POST.get("target")
    ...
    for target_key in sorted(stage_targets.keys()):
        display_label = target_label_overrides.get(target_key, target_key.replace("_", " ").title())
        tool_nav.append(
            {
                "key": target_key,
    ...
    if request.method == "POST" and active_section == "providers":
        ...
        if action == "provider-upsert":
                ...
                if not cred:
                    ...
                    if not provider_obj:
                        ...
                        if desired and not analysis.get("ready"):
                            issues = analysis.get("issues") or ["Provider validation failed."]
            ...
            if not provider_key:
                ...
                stored_api_key = secret_details.get("api_key") if secret_details else ""
                stored_metadata = secret_details.get("metadata") if secret_details else {}
                effective_endpoint = (
                    form_data.get("endpoint")
                    or ""
    ...
    if request.method == "POST" and active_section != "providers":
        ...
        if target == "guardian" and action in {
            ...
            if action == "instruction-add":
                ...
                filtered = [
                    entry for entry in instructions if str(entry.get("id")) != instruction_id
                ]
        ...
        if action == "config-delete":
            ...
            if config_id:
                delete_llm_configuration(organization_id=str(organization.id), config_id=config_id)
            ...
            for stage_key in stage_keys:
                provider_value = (
                    (request.POST.get(_stage_field(stage_key, "provider")) or "").strip().lower()
                )
                ...
                if temperature_raw:
                    try:
                        entry.setdefault("options", {})["temperature"] = float(temperature_raw)
                    except ValueError:
        ...
        for name in configured_names:
            ...
            for model_meta in models_payload:
                if not isinstance(model_meta, dict):
                    continue
                options_meta = (
                    model_meta.get("options") if isinstance(model_meta.get("options"), dict) else {}
                )
                combined = {
                    **model_meta,
            ...
            provider_list.append(
                {
                    ...
                    "label": cred_entry.get("display_name") or registry_entry.get("label") or name,
                    "status": status_value,
                    "endpoint": cred_entry.get("endpoint") or registry_entry.get("endpoint"),
    ...
    for stage_key in stage_keys:
        ...
        selected_options = (
            selected.get("options") if isinstance(selected.get("options"), dict) else {}
        )
        stage_entries.append(
            {
                ...
                or (assignment.providers[0] if assignment and assignment.providers else ""),
                ...
                "field_opt_chunk_overlap_tokens": _stage_field(
                    stage_key, "opt_chunk_overlap_tokens"
                ),
...
def _unique_configuration_name(organization_id: str, target: str, desired: str) -> str:
    ...
    existing = set(
        LLMConfiguration.objects.filter(organization_id=organization_id, target=target).values_list(
            "name", flat=True
        )
    )
