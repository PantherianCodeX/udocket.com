...
- **Constitution Alignment**: Ensure tasks cover typed primitives + schemas (P1), AI runtime isolation (P2),
  deterministic manifests/fixtures (P3), ≥90 % coverage + prompt evals (P4), OTLP/ops JSONL/RTO metrics (P5), and
  security/performance/migration/accessibility tasks (P6).
...
- [ ] T007 Create typed models/entities + provider stubs in `packages/common/` per Constitution P1
- [ ] T008 Define Pydantic models + exported JSON Schemas for external consumers
- [ ] T009 Configure AI runtime client wiring (`packages.ai.api`, Portkey) and residency guardrails
- [ ] T010 Configure structlog + OpenTelemetry exporters and append-only ops JSONL writers
- [ ] T011 Establish fixture library in `tests/fixtures/` and baseline prompt evaluation harness
- [ ] T012 Author threat model + security scan plan (STRIDE/abuse cases, secrets handling)
- [ ] T013 Document performance budgets (p95 latency/throughput/cost) and plan load/chaos tests
- [ ] T014 Draft forward-only migration notes (dev mode) + DR runbook entries (RTO/RPO, rollback if needed, incident contacts)
- [ ] T015 Setup environment configuration management and compliance docs (data inventory, retention)
