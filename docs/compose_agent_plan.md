...
- Optional staff report, timeline seeds, and entity hints from the Summarize agent.
...
3. **LLM `compose.context_builder`**
   - Summarize intake details, key issues, parties, and outstanding questions into a compact brief.
...
## Next Steps
1. Implement `packages/udocket_core/agents/compose_lib.py` mirroring the Summarize agent structure with stage orchestrator and deterministic writers.
