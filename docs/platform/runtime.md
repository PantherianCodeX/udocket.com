...
- **Maintenance:** Run `python -m doc_tools.manage_docs --lint docs/platform/runtime.md docs/overview/tdd.md docs/tdd_modularization.md` plus `python -m doc_tools.build.runbook_catalog --check` before landing infra changes. Update diagrams via `uv run --project packages/udocket_docs python -m doc_tools.render_mermaid` whenever topology shifts.
...
- Container runtime: production runs on AKS with Flux CD applying Helm releases; PodSecurity admission enforces the restricted baseline (no privileged pods, `readOnlyRootFilesystem` where feasible), and cosign attestations gate image promotion. Local development uses `make stack.up` (wrapping `docker compose -f docker-compose.yml -f docker-compose.dev.yml -f docker-compose.cache.yml up -d`) to mirror service topology and health checks while sharing the `.env` schema.
