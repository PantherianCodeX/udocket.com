...
- **Maintenance:** Run `python -m doc_tools.manage_docs --lint docs/platform/identity.md docs/overview/tdd.md docs/tdd_modularization.md` before landing identity changes. Update runbooks via `python -m doc_tools.build.runbook_catalog --check`, rerun `make lint-db` when modifying RLS or masking SQL.
...
**Breadcrumbs:** Ops catalog, runbooks `docs/ops/runbooks/identity/*.md`, automation scripts. **|**
