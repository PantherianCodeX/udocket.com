...
- **Maintenance:** Run `python -m doc_tools.manage_docs --lint docs/customer/communications.md docs/overview/tdd.md docs/tdd_modularization.md` before submitting changes. Updates that alter schema, queue semantics, or provider adapters also require `python -m doc_tools.build.runbook_catalog --check` to pass. Notify Platform + Ops architecture lists on PRs.
...
**Observability:** Docs lint (`python -m doc_tools.build.runbook_catalog --check`), dashboards “Notifications Delivery” / “In-App Notifications”, alert `alert_notifications_delivery_health`. **|**
