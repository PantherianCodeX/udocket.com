...
- **Maintenance:** Run `python -m doc_tools.manage_docs --lint docs/customer/accounts-tenants.md docs/platform/identity.md docs/overview/tdd.md` before submitting. Changes to lifecycle stages require updates to TDD §14.1 checklists and runbook catalog entries.
...
**State:** Stored in `docs/customer/accounts-tenants/error_codes.yaml`. **|**
...
**Breadcrumbs:** Runbooks `docs/ops/runbooks/accounts/*.md`, automation `scripts/ops/tenant_*.py`, staffing roster `ops/tenants/rota.md`. **|**
...
**Breadcrumbs:** Incident response guide `docs/ops/runbooks/accounts/README.md`, staffing policy `ops/tenants/policies/staffing.md`. **|**
