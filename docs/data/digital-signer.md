...
header-includes:
  ...
  - |
    <header class="page-header">uDocket — Digital Signer Technical Design <br> Platform Signing, Trust Roots, and Attestation Specification</header>
  - |
...
- **Maintenance:** Run `python -m doc_tools.manage_docs --lint` and `python -m doc_tools.check.links --strict` before proposing signer changes. Signing policy or PKI updates require ADR references in the PR.
...
**Breadcrumbs:** Observability config `infra/observability/dashboards/signer.json`, Prometheus rules `infra/monitoring/signer-prometheus-rules.yaml`, runbooks `docs/ops/runbooks/signer/*.md`. **|**
