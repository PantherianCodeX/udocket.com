...
- **Maintenance:** Run `python -m docs.tools.manage_docs --lint docs/src/automation/worker-cluster.md docs/src/overview/tdd.md docs/tdd_modularization.md` plus `python -m docs.tools.build.runbook_catalog --check` before landing worker changes. Update task-module AGENTS guides when adding queues or long-running jobs.
...
**Observability:** Docs lint (`python -m docs.tools.build.runbook_catalog --check`), dashboards “Worker Queues”/“Watchdog Runner”, alert `watchdog_runner_missed_total`. **|**
