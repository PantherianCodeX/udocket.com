...
**Observability spec:** [`../services/logging.md`](../services/logging.md) governs schema, pipeline topology, trace correlation, access controls, redaction, sampling, and cost guardrails. Key platform metrics continue to include `logging_ingest_lag_seconds`, `logging_drop_rate_pct`, `trace_sampling_rate`, and `logging_volume_budget_violation_total`.
