...
- **Maintenance:** Run `python -m docs.tools.manage_docs --lint docs/src/platform/identity.md docs/src/overview/tdd.md docs/tdd_modularization.md` before landing identity changes. Update runbooks via `python -m docs.tools.build.runbook_catalog --check`, rerun `make lint-db` when modifying RLS or masking SQL.
