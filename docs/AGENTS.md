...
- **Tooling requirements.** Run commands through the provided containers/venvs (`make ...`, `uv run --project …`). Never install ad-hoc dependencies via `pip`. Docs/spec changes must pass `doc_tools.check.links` and MkDocs builds.
