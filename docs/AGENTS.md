...
- **Tooling requirements.** Run commands via the provided containers/venvs
  ...
  Before merging any AI runtime change, run `make ai.mypy`, `make ai.pyright`,
  and `make ai.ruff` so dependencies resolve consistently and typing/linting
  cover the package with the correct configs. Docs/spec changes must pass
