...
def render_groups(groups: Sequence[OwnerGroup]) -> str:
    lines: list[str] = []
    lines.append("<!-- AUTO-GENERATED: Run `python -m docs.tools.build.diagram_index` to refresh. -->")
    lines.append("")
    if not groups:
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    ok = write_or_check(
        ...
        updated_content,
        check=args.check,
        stale_message="Diagrams index is stale. Run `python -m docs.tools.build.diagram_index` to refresh.",
    )
